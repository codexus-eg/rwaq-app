...
fun NewRewardScreen(viewModel: NewRewardViewModel = koinViewModel()) {
    ...
    DisposableEffect(lifecycleOwner) {
        val observer = LifecycleEventObserver { _, event ->
            if (event == Lifecycle.Event.ON_RESUME) {
                viewModel.checkAuthenticationAndFetchData()
            }
        }
    ...
    EventHandler(viewModel.effect) { effect, controller ->
        when (effect) {
            is RewardsUiEffect.NavigateBack -> controller.navigateUp()
            is RewardsUiEffect.NavigateToLogin -> controller.navigate(Screen.LoginScreen)
            else -> {}
        }
    }
...
@Composable
fun NewRewardProductGrid(
    products: List<com.khater.rwaq.domain.entities.product.Product>,
    points: Double,
    ...
    androidx.compose.foundation.lazy.grid.LazyVerticalGrid(
        ...
        items(products, key = { it.id }) { product ->
            com.khater.rwaq.presentation.screens.rewardScreen.components.RewardProductItem(
                product = product,
                points = points,
