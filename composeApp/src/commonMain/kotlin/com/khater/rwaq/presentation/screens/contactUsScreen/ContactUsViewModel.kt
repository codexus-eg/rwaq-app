...
class ContactUsViewModel constructor(
    private val settings: FlowSettings,
    private val sendContactMessageUseCase: SendContactMessageUseCase,
) : BaseViewModel<ContactUsUiState, ContactUsUiEffect>(ContactUsUiState()),
...
//        // Simulating fetching data from Profile/Local User
//        updateState {
//            it.copy(
//                phoneNumber = "+966534746088" // Replace with actual data
//            )
//        }
//    }
     ...
     private fun getUser() {
        viewModelScope.launch {
            ...
            val phone = settings.phoneNumber.first()
            updateState { it.copy(username = username, phoneNumber = phone) }
        }
    }
    ...
    override fun onMessageChanged(message: String) {
        // Clear error when user types
        updateState { it.copy(message = message, messageError = null) }
    }
    ...
    override fun onBackClicked() {
        sendNewEffect(ContactUsUiEffect.NavigateBack)
    }

    private fun validateInput(): Boolean {
        val currentEmail = state.value.email
        val currentUsername = state.value.username
        val currentMessage = state.value.message
        var isValid = true
        ...
        if (currentMessage.isBlank()) {
            updateState { it.copy(messageError = Res.string.message_required) }
            isValid = false
        }
    ...
    private fun sendMessage() {
        ...
        tryToExecute(
            ...
            onSuccess = {
                updateState { it.copy(isLoading = false,errorMessage = null, message = "", username = "", email = "") }
                showSnackBar(
                    title = getString(Res.string.success),
                    message = getString(Res.string.message_sent_success),
                    isSuccess = true
                )
                 sendNewEffect(ContactUsUiEffect.NavigateBack)
            },
            onError = { error ->
                showSnackBar(
                    ...
                    message = getString(Res.string.message_sent_failed),
                    isSuccess = false
                )
